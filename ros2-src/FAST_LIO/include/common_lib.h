...
inline rclcpp::Time get_ros_time(double sec)
{
    int32_t s  = static_cast<int32_t>(sec);
    uint32_t ns = static_cast<uint32_t>((sec - s) * 1e9);
    return rclcpp::Time(s, ns, RCL_ROS_TIME);
}
